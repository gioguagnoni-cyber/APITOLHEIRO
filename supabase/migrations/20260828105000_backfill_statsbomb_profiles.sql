-- Seeds profiles from aggregates already imported before the service-only
-- profile refresh was corrected. On a clean project this is a harmless no-op.
...
with team_rows as (
  select home_team_key as team_key, home_team_name as team_name, match_date,
    home_score::numeric as goals_for, away_score::numeric as goals_against,
    home_xg as xg_for, away_xg as xg_against, home_shots::numeric as shots_for,
    away_shots::numeric as shots_against, home_shots_on_target::numeric as shots_on_target_for,
    home_completed_passes::numeric as completed_passes, home_pressures::numeric as pressures,
    events_ingested_at
  from public.statsbomb_matches
  where events_ingested_at is not null
  union all
  select away_team_key, away_team_name, match_date,
    away_score::numeric, home_score::numeric, away_xg, home_xg,
    away_shots::numeric, home_shots::numeric, away_shots_on_target::numeric,
    away_completed_passes::numeric, away_pressures::numeric, events_ingested_at
  from public.statsbomb_matches
  where events_ingested_at is not null
)
select
